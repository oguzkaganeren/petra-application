 ...
 * Location state
 */
export interface GetAllRestaurantTypesState {
	selected: any;
 ...
 * Location component
 */
export class GetAllRestaurantTypesComponent extends React.Component<
	GetAllRestaurantTypesProps,
	GetAllRestaurantTypesState
> {
	/**
						...
						if (data) {
							data.RestaurantType.map(dat => {
								if (this.state.datam.length > 0) {
									if (this.state.datam.every(item => item.id !== dat.restaurantTypeID)) {
										this.state.datam.push({ id: dat.restaurantTypeID, text: dat.type });
									}
								} else {
									this.state.datam.push({ id: dat.restaurantTypeID, text: dat.type });
								}
